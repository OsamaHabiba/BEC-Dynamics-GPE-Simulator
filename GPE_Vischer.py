...
from numpy import pi, sqrt, exp, linspace, meshgrid, zeros, empty, random, cosh, arctan2, cos, sin
import numpy
...
class Potential(object):
    ...
    def laser_swipe(self, x, y, t):
        ...
        laser = A*exp(-(xs*xs + ys*ys) / w**2)
        return harmonic + laser



def x_ax(wavefunction):
    new = []
    columns = len(wavefunction[0])
    half = round(columns/2)

    for j in range(columns-1):
        new.append(wavefunction[half][j])
...
class Wavefunction:
    ...
    def __init__(self, init_func, x, y, dt):
        ...
        dx = x[0,1] - x[0,0]
        N = sqrt(abs(self.norm()).sum() * dx**2/10)
        self.real /= N
        self.imag /= N
        self.prev /= N
    ...
    def angle(self):
        ...
        return numpy.angle(self.real + 1j*self.imag)
...
def energy(psi,dx):
    grad_x = numpy.gradient(psi.func(), dx, axis=0)
    grad_y = numpy.gradient(psi.func(), dx, axis=1)
    grad_squared = 0.5 * (numpy.abs(grad_x)**2 + numpy.abs(grad_y)**2)
    potential_energy = sim.V(sim.x, sim.y, sim.time) * psi.norm()
    quartic_term = sim.eta * psi.norm()**2
    energy = numpy.trapz(grad_squared + potential_energy + quartic_term, dx=dx, axis=0)
    total_energy = numpy.trapz(energy, dx=dx)
    return total_energy
...
def animate(simulation, time, termination_time, interval=100):
    ...
    elapsed_time = 0.0  # Track the elapsed time

    def update(i):
        nonlocal elapsed_time  # Use the outer elapsed_time variable
        ...
        #T.append(simulation.time)
        #E.append(energy(wf, simulation.dx))
        #x_axis.append(x_ax(wf.norm()))
        #print(x_ax(wf.norm()))

        elapsed_time += time / interval  # Update the elapsed time

        if elapsed_time >= termination_time:
            anim.event_source.stop()  # Stop the animation if termination time is reached
...
if __name__ == '__main__':
    params = {'N': 150,
              ...
              'BC': 'reflect',
              'nonlinearity': 1,
              'initial': GaussianWavepacket(2,0,0) ,
              'potential': 'laser_swipe',
              }
...
#dataE = numpy.column_stack((T, E))

#numpy.savetxt('energy_blue_v(5).dat', dataE, delimiter='\t')


'''
x_co = linspace(-sim.xmax , sim.xmax, 2*round(sim.xmax/sim.dx))
for i in range(0 , len(x_axis), 1):
       file_name = "{}_eta({})_t({})_theta2d.dat".format(params['potential'],params['nonlinearity'],i)
       data = numpy.column_stack((x_co, x_axis[i]))
...
file_name = "{}_eta({})_t(1)_theta2d.dat".format(params['potential'],params['nonlinearity'])
data = numpy.column_stack((x_co, x_ax(sim.wf.norm())))
numpy.savetxt(file_name, data, delimiter='\t')

'''

function_values = sim.wf.norm()
...
x_coords, y_coords = sim.x, sim.y
  
# Reshape 2D array into 1D array
function_values_flat = numpy.ravel(function_values)
#theta_flat = numpy.ravel(theta)

# Combine x, y, and flattened 2D array into a single 3-column array
data = numpy.column_stack((x_coords.flatten(), y_coords.flatten(), function_values_flat))

file_name = "v_(5){}_eta({})_t({})_theta2d.dat".format(params['potential'],params['nonlinearity'],sim.time)
